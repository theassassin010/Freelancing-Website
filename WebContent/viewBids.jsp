		...
		function getBidDetails(taskid){
			$.ajax({
			    ...
			    success : function(result) {
			    	...
			    	var output = "<h1><strong> Bids on the project: </strong></h1> <br>";
			    	output += "<table><tr><th>S. No.</th><th>Name</th><th>Proposed Completion Date</th><th>Amount</th><th>Rating</th><th>Accept Bid</th></tr>";
			   		for(i=0; i<json_obj.length; i++){
						var bid = json_obj[i];
			   			...
			   			output += "<input type=\"hidden\" value=\""+taskid+"\" name=\"taskid\" />";
			   			output += "<input type=\"hidden\" value=\""+bid.fid+"\" name=\"fid\" />";
			   			output += "<input type=\"hidden\" value=\""+bid.comp_ts+"\" name=\"comp_ts\" />";
	   					output += "<input type=\"submit\" class=\"button\" name=\"assignProject\" value=\"Accept Bid\" style=\"float: center;\">";
						output += "</form></td></tr>";
			   			console.log(output);
			   		}
			   		output += "</table>";
		...
		function getTaskInfo(taskid){
			$.ajax({
			    ...
			    success : function(result) {
					...
					$("#subject").html(json_obj.subject);
					$("#text").html(json_obj.text);
					$("#date").html(json_obj.comp_ts);
					$("#range").html(range);
				},
			    error : function() {
			    	alert("Error Occured");
...
<body>
	...
	<fieldset>
		 ...
		 <h2 align = "left"> To be completed by: </h2>
